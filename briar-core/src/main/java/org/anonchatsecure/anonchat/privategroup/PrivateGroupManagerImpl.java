...
import org.anonchatsecure.anonchat.api.privategroup.MessageType;
...
import static org.anonchatsecure.anonchat.api.privategroup.Visibility.VISIBLE;
import static org.anonchatsecure.anonchat.privategroup.GroupConstants.GROUP_KEY_CREATOR_ID;
import static org.anonchatsecure.anonchat.privategroup.GroupConstants.GROUP_KEY_DISSOLVED;
	...
	private void applyRetentionToExistingMessages(Transaction txn,
		...
		try {
			...
			for (Entry<MessageId, BdfDictionary> entry : metadata.entrySet()) {
				...
				BdfDictionary meta = entry.getValue();
				// Skip JOIN messages
				if (meta.getInt(KEY_TYPE) == JOIN.getInt()) continue;
				long timestamp = meta.getLong(KEY_TIMESTAMP);
				if (durationMs == -1) {
					deadlines.put(id, -1L);
				} else {
	...
	public void deleteMessages(Transaction txn, GroupId g,
		...
		for (MessageId m : messageIds) {
			try {
				...
				// metadata already deleted, safe to proceed
			}
