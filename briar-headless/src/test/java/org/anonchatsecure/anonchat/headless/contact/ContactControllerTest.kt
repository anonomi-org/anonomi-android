...
import org.anonchatsecure.bramble.api.contact.PendingContactState.FAILED
import org.anonchatsecure.bramble.api.contact.PendingContactState.WAITING_FOR_CONNECTION
...
internal class ContactControllerTest : ControllerTest() {
    ...
    fun testAddPendingContactSamePendingContactKey() {
        ...
        } throws PendingContactExistsException(
            pendingContact
        )
        every {
            ctx.json(
    ...
    fun testSetContactAlias() {
        ...
        every { contactManager.setContactAlias(
            ContactId(
                1
            ), "foo") } just Runs
    ...
    fun testDelete() {
        every { ctx.pathParam("contactId") } returns "1"
        every { contactManager.removeContact(
            ContactId(
                1
            )
        ) } just Runs
